...
import {
  ...
  FaComments, FaEyeSlash, FaEye, FaCamera, FaBuilding, FaLock, FaSpinner, FaCreditCard,
} from 'react-icons/fa';
import {
...
// ChatTab is imported from '../components/ChatTab'
...
function DashboardOverviewTab({ user, onNavigate }) {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const [myEnrollments, myProfile, gamificationData, occupancyData, allPlans] = await Promise.all([
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 16 }}>
      ...
      {plans.map(p => (
        <Card key={p.id} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {hasActiveMembership ? (
            <div style={{
              textAlign: 'center', padding: '10px', background: '#f0fdf4', border: '1px solid #86efac',
      ...
      <div style={{ marginLeft: 76, flex: 1, minHeight: '100vh', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 20px', width: '100%', boxSizing: 'border-box' }}>
          <Routes>
            ...
            <Route path="profile" element={<ProfileDietTab onLogout={onLogout} />} />
